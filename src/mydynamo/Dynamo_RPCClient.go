...
func (dynamoClient *RPCClient) Get(key string) *DynamoResult {
	fmt.Println("GET CALLED from client side")
	var result DynamoResult
	if dynamoClient.rpcConn == nil {
		fmt.Println("GET CALLED no connection")
		return nil
	}
	fmt.Println("GET CALLED from client side: Calling")
	err := dynamoClient.rpcConn.Call("MyDynamo.Get", key, &result)
	if err != nil {
		fmt.Println("err while called GET", err)
		log.Println(err)
		return nil
	}
	fmt.Println("GET CALLED from client side: Returning")
	return &result
}
